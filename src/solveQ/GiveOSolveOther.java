...
public class GiveOSolveOther {
    public static void main(String[] args) {
        ...
        // 커스텀 패널 생성
        GradientOtherPanel panel = new GradientOtherPanel();
        frame.setContentPane(panel); // 커스텀 패널을 프레임의 컨텐츠 패널로 설정
        frame.setVisible(true); // 프레임 출력
    }

    // 기타 화면용 그라데이션과 이미지를 그리는 커스텀 JPanel 클래스
    static class GradientOtherPanel extends JPanel {
        ...
        private String otherAdvice; // 랜덤 조언을 저장할 변수

        public GradientOtherPanel() {
            try {
                // 이미지 로드
                ...
                // 랜덤 기타 조언 로드
                otherAdvice = getRandomAdvice("keywordText/KeywordOther.txt");
            } catch (IOException e) {
                e.printStackTrace();
                otherAdvice = "기타 조언을 불러오지 못했습니다.";
            }
        }
        ...
        protected void paintComponent(Graphics g) {
            ...
            // 텍스트 그리기 (요청된 위치 유지)
            drawAdviceText(g, otherAdvice, width, height);
        }
